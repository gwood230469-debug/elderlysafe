export const copy = {
  ...
  safeword: {
    ...
    suggestionPrompt: (word: string) => `Stuck? Try: "${word}"`,
    useSuggestion: 'Use this',
    anotherSuggestion: 'Another idea',
    rotationReminder: (days: number) =>
      `Your safe word has been the same for ${days} days. Many families change theirs every few months — no rush, just something to consider.`,
    rotationFresh: (days: number) => `Your safe word was last changed ${days} ${days === 1 ? 'day' : 'days'} ago.`,
  },
  circle: {
  ...
  rehearsal: {
    notificationTitle: 'Practice run',
    notificationBody: (name: string) => `Pretend ${name} is calling and needs money right now — what do you do?`,
    title: 'Just a practice run',
    subtitle: "No one's actually calling. This is here so the real steps feel familiar if you ever need them.",
    scenario: (name: string) => `Imagine: someone calling claiming to be ${name}, saying they're in trouble and need money right away.`,
    completeButton: "I know what I'd do",
    completedTitle: "Nice — you're ready.",
    completedBody: 'Keep your safe-word card somewhere handy, just in case.',
  },
  dashboard: {
    title: 'SafeWord',
    familyCircleLabel: 'Family circle',
    readinessInformed: (date: string) => `Safe word confirmed ${date}`,
    readinessNotInformed: 'Safe word: needs to be told',
    markAsTold: 'Mark as told',
    readinessNeverRehearsed: 'Never rehearsed',
    readinessLastRehearsed: (days: number) => `Last rehearsed ${days === 0 ? 'today' : `${days} ${days === 1 ? 'day' : 'days'} ago`}`,
    sendPracticeRun: 'Send a practice run',
    ...
    rotationBanner: (days: number) => `Your family's safe word hasn't changed in ${days} days.`,
    outcome: {
      declined: 'Declined',
